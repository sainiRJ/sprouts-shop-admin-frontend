...
import { revenueData, orderStatusData, mockOrders, mockProducts } from "@/data/mockData";

const stats = [
  { label: "Total Revenue", value: "₹1,98,600", change: "+12.5%", up: true, icon: DollarSign },
  { label: "Total Orders", value: "228", change: "+8.2%", up: true, icon: ShoppingCart },
  { label: "Active Users", value: "1,429", change: "+5.1%", up: true, icon: Users },
  { label: "Products", value: "477", change: "-2.3%", up: false, icon: Package },
];
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
        ...
        <div className="glass-card p-5 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={revenueData}>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: "hsl(0,0%,100%)", border: "1px solid hsl(220,14%,90%)", borderRadius: "8px", color: "hsl(220,20%,14%)" }}
              />
        ...
        <div className="glass-card p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              ...
              <Tooltip contentStyle={{ backgroundColor: "hsl(0,0%,100%)", border: "1px solid hsl(220,14%,90%)", borderRadius: "8px", color: "hsl(220,20%,14%)" }} />
            </PieChart>
          </ResponsiveContainer>
      ...
      <motion.div variants={item} className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="glass-card p-5">
          ...
          <div className="space-y-3">
            {mockProducts.filter((p) => p.stock <= 20).map((p) => (
              <div key={p.id} className="flex items-center justify-between text-sm">
                ...
                <span className={`text-xs font-semibold ${p.stock === 0 ? "text-destructive" : "text-warning"}`}>
...
function OrderBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize ${colors[status] || ""}`}>
      {status}
    </span>
  );
}
